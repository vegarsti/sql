...
func TestSelectJoin(t *testing.T) {
	input := "select a from foo join bar on foo.a = bar.b"
	l := lexer.New(input)
	p := parser.New(l)
	...
	if stmt.Join == nil {
		t.Fatalf("stmt.Join is nil")
	}
	expectedJoinTable := "bar"
	if stmt.Join.Table != expectedJoinTable {
		t.Fatalf("stmt.Join.Table is not %s. got=%s", expectedJoinTable, stmt.Join.Table)
	}
	expectedJoinExpr := "(foo.a = bar.b)"
	if stmt.Join.On.String() != expectedJoinExpr {
		t.Fatalf("stmt.Join.On is not %s. got=%s", expectedJoinExpr, stmt.Join.On)
	}
}
